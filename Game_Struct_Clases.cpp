...
void Enemigo::initVariables()
{
    this->hp = this->hpBase;
    this->dmg = this->dmgBase;
    this->lvl = this->lvlMin;
    this->velocidadMovimiento = 0.8f;  // mas lenta que el jugador
}

void Enemigo::initSprite()
{
    if (!this->textureSheet.loadFromFile("Sprites ejemplo/enemy_sprites.png")) ///NO TENGO EL SPRITE
    {
        std::cout << "ERROR::ENEMIGO::No se pudo cargar textura del enemigo.\n";
    }
    this->sprite.setTexture(this->textureSheet);
    this->sprite.setScale(2.5f, 2.5f);
    this->sprite.setPosition(400.f, 400.f); // tester posición inicial
}

Enemigo::Enemigo(int id, int hp, int dmg, int lvlMin, int lvlMax, std::string nombre)
    : Personajes(id, hp, dmg, lvlMin, nombre)
{
    this->idEnemigo = id;
    this->hpBase = hp;
    this->dmgBase = dmg;
...
{
    // Movimiento muy básico de ejemplo (luego se puede mejorar)
    float dx = static_cast<float>((rand() % 3) - 1); // -1, 0 o 1
    float dy = static_cast<float>((rand() % 3) - 1);

    this->sprite.move(dx * this->velocidadMovimiento, dy * this->velocidadMovimiento);
}

void Enemigo::update()
{
    this->moverAleatorio();
    // agregar animaciones, IA, detección de jugador, etc.
}
