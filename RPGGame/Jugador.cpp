...
{
    for(auto i : inventory) if(i == item) return true;
    return false;
}
...
void Jugador::updateMovement()
{
    ...
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Space) && hasItem(ITEM_DASH))
    {
        if(this->dashTimer.getElapsedTime().asSeconds() > 7.0f)
        {
            ...
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::A)) this->dashDir = sf::Vector2f(-1.f, 0.f);
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::W)) this->dashDir = sf::Vector2f(0.f, -1.f);
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::S)) this->dashDir = sf::Vector2f(0.f, 1.f);
        }
    }
...
void Jugador::atacar(Character *enemigo)
{
    ...
    // La logica real de daño se maneja en Habitacion::update usando getAttackHitbox
...
void Jugador::updateAnimations()
{
    if (this->animState == PLAYER_ANIMATION_STATES::ATTACK)
    {
        if (this->animationTimer.getElapsedTime().asSeconds() >= 0.1f || getAnimSwitch())
        {
            ...
            if(this->currentFrame.left >= 60.f) // 3 frames (0, 20, 40)
            {
                this->animState = PLAYER_ANIMATION_STATES::IDLE;
                this->currentFrame.left = 0.f;
                this->resetAttack(); // Clear hit list for safety
            }
...
void Jugador::update()
{
    ...
    if(this->isAttacking()) {
        ...
        this->debugHb.setFillColor(sf::Color(255, 0, 0, 100)); // Rojo semitransparente
...
{
    std::vector<int> inv;
    for(auto i : inventory) inv.push_back((int)i);
    return inv;
}

int Jugador::getCoins() const
{
    return this->coins;
}
int Jugador::getKeys() const
{
    return this->keys;
}
int Jugador::getMaxHp() const
{
    return this->maxHp;
}

void Jugador::setStats(int hp, int maxHp, int coins, int keys, const std::vector<int>& inv)
{
    this->hp = hp;
    this->maxHp = maxHp;
    this->coins = coins;
    ...
    for(int i : inv) this->inventory.push_back((ItemType)i);
}

void Jugador::resetAttack()
{
...
void Jugador::addHit(Character* target)
{
    this->hitEnemies.push_back(target);
}
...
sf::FloatRect Jugador::getAttackHitbox() const
{
    ...
    {
        return sf::FloatRect(swordHb.left + swordHb.width/2 - width/2, swordHb.top + swordHb.height, width, range);
    }
    else if(facingDirection == 1)   // Left
    {
        return sf::FloatRect(swordHb.left - range, swordHb.top + swordHb.height/2 - width/2, range, width);
    }
    else if(facingDirection == 2)   // Right
    {
        return sf::FloatRect(swordHb.left + swordHb.width, swordHb.top + swordHb.height/2 - width/2, range, width);
    }
    else if(facingDirection == 3)   // Up
    {
        return sf::FloatRect(swordHb.left + swordHb.width/2 - width/2, swordHb.top - range, width, range);
    }
